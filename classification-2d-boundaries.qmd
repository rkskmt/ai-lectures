...
- 判定ルール：**輝度 < 5.5 → Salmon、5.5 以上 → Sea bass**（Salmon を陽性とする）
- 前の図で、線からはみ出した点を数えて、**混同行列を埋めよ**（各クラス40匹）
   - 線の**右側**の黒丸 … 見逃した Salmon（FN）
   - 線の**左側**の赤四角 … Salmon 扱いした Sea bass（FP）
   - 残りが TP と TN

::: {.fragment .table-compact}
答え：
...
TODO：ここに具体的アルゴリズムを書く。いま、閾値の話とクラスの話と、分割後の領域の中での閾値探索の話がごっちゃになっている。図から見直し
...
direction: right
C: "分割される閾値候補を\n全部試して、各閾値のジニ不純度を記録" { style.fill: "#e1f5ff" }
G: "ジニ不純度が\n最小の分割を採用" { style.fill: "#fff4e1" }
R: "２つの特徴で\n繰り返し" { style.fill: "#fff4e1" }
E: "N回（深さ上限） or\nこれ以上きれいにならない\n→ 終了" { style.fill: "#d5f5e3" }
C -> G -> R
R -> C: 次の段へ {style.stroke-dash: 3}
R -> E
```
...
TODO:scikit-learnのimportはここでやる。前にかいてるのは消せ
...
comparison_df = pd.DataFrame({
    "モデル": ["手動の縦線（輝度 5.5）", "決定木（深さ3）"],
    'Accuracy': [f"{accuracy_hand:.3f}", f"{accuracy_tree:.3f}"],
    'Precision': [f"{precision_hand:.3f}", f"{precision_tree:.3f}"],
    'Recall': [f"{recall_hand:.3f}", f"{recall_tree:.3f}"]
})
