---
title: "機械学習 Classification"
subtitle: "最も単純な２クラス分類"
---
...
- 青と赤のデータを分離する**境界**を描きたい

::: {.callout-important title="単純に考えると"}
直線では分離不可能
:::
...
- 分類問題(Data Classification)：データを分類するタスクを理解する
    - 2クラス分類
- データ可視化
- **決定境界（[decision boundary]{.en}）** の概念を学ぶ
...
- サケ以外の魚も混じっている
  ...
  - 分類項目を**クラス（[class]{.en}）**という
:::
::: {.column width="40%"}
![](imgs/2026-02-04-12-03-25.jpg){fig-align="center" width="80%"}
:::
:::
...
direction: down
A: ベルトコンベアに魚を流す { style.fill: "#e1f5ff" }
B: 途中で重さを測定 { style.fill: "#fff4e1" }
D: 何かしらの判定器 { shape: diamond; style.fill: "#ffd9d9" }
E: 鮭として加工 { style.fill: "#ffcccb" }
F: スズキとして加工 { style.fill: "#add8e6" }
A -> B -> D
D -> E: Salmon
D -> F: Sea bass
```
:::
::: {.column width="40%"}
![](imgs/2026-02-04-11-46-47.jpg){fig-align="center"}
:::
:::
...
## ここまでで分かること
- **双峰型の分布**：2つの山が見える
  -  **かなり重なっている** が、まあ分けてみましょう
:::

## 境界線（閾値）を考える

大体でいいので **しきい値（[threshold]{.en}）** を設定する。
...
| 実際の魚 | 判定結果 | 略称 | 説明 |
|----|----|------|--------|
| Salmon | Salmon | **TP** (True Positive) | **正しくSalmon**と判定 |
| Sea bass | Sea bass | **TN** (True Negative) | **正しくSalmonではない**と判定 |
| Sea bass | Salmon | **FP** (False Positive) | **誤ってSalmon**と判定 |
| Salmon | Sea bass | **FN** (False Negative) | **Salmonを見逃し** |
...
```{.d2 sketch="true" width="78%"}
direction: right
J: "判定器の出力" { style.fill: "#e1f5ff" }
P: "Positive\n（Salmon である）" { style.fill: "#fff4e1" }
N: "Negative\n（Salmon ではない）" { style.fill: "#fff4e1" }
TP: "TP" { style.fill: "#d5f5e3" }
FP: "FP" { style.fill: "#f5b7b1" }
TN: "TN" { style.fill: "#d5f5e3" }
FN: "FN" { style.fill: "#f5b7b1" }
J -> P
J -> N
P -> TP: 当たり
P -> FP: はずれ
...
## 混同行列（[Confusion Matrix]{.en}） {#peek-cm-1800}
...
:::{.callout-important}
- **決定境界の閾値を下げる（1600g）** → Recall ↑、Precision ↓  
  - Salmonを逃さない（FN減）が、Sea bassの誤判定（FP増）
  
- **決定境界の閾値を上げる（2000g）** → Precision ↑、Recall ↓  
...
## コストを考慮した最適化 {#peek-cost}

誤分類のコストが異なる場合、総コストを最小化するしきい値を選べる。
$$
\begin{aligned}
...
## **Tweak** しきい値を振り回す {.badge-practice}

- [混同行列のコード](#peek-cm-1800){.peek}の `threshold` と、[コスト最適化のコード](#peek-cost){.peek}を**書き換えて実行**（指標のセルまで続けて実行）

::: {.tweak}
| やりたいこと | コード |
|--|--|
| 全部 Salmon と言い張る | `threshold = 1000` |
| 全部 Sea bass と言い張る | `threshold = 2500` |
| 再選別のほうが高くつく世界 | `C_FP = 500`、`C_FN = 10000` |
:::
...
2. **境界線は「方針の選択」**
   - 正解/不正解ではなく、用途に応じた最適化（[optimization]{.en}）
   
3. **評価指標はトレードオフ**
   - 例：Precision ↑ だと Recall ↓
   - どちらを重視するかは用途次第
:::
