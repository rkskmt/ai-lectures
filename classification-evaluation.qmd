...
- `accuracy_score` などで精度を評価した
- モデルが学習データに対して高い精度を示した
...
::: {.callout-warning}
## テストデータの鉄則
テストデータを確認してからモデルを調整してはいけない。
「見たことのない問題」を模擬するためのもの。
:::
...
- `train_test_split` は**ランダムに分割**するため、精度が変わってしまう
...
::: {.callout-warning}
## データが少ないほど揺れが大きい
テストデータが16匹の場合 → 1匹の誤分類で精度が 6% 変わる
:::
