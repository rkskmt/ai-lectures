...
- 例えばこの線：正答率 **91%**（縦線は 86% だった）
- **この線さえあれば、判別は「線の上か下か」だけ** — これが**決定境界**
- ただし「斜め」は無限にある。最適の保証もない — **機械に求めさせたい**
...
- 前回の線形回帰と**見た目はそっくり**。でも役割は**正反対**
- **回帰直線**：点の**真ん中**を通る — 縦軸の値（重さ）を**予測する**線
- **決定境界**：点の**間**を通る — 2クラスを**分ける**線。予測したい「魚の種類」は軸ではなく**色**で乗っている
...
- 「最適な直線で、上か下かを判別する」機械 — それが**ロジスティック回帰**
- 中身は**2ステップ**：Step 1 が**直線**、Step 2 が**確率** — 1つずつ開ける
...
- 数学でおなじみの**直線の式 $ax + by + c = 0$** そのもの — さっきの斜めの線の正体はこれ（決定境界）
